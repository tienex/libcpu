/** @file
  Z shell textual-emit backend. See ZshBackend.h.

  Each ICpuValue is a zsh integer variable (tN) masked to its width; arithmetic
  uses $(( ... )). zsh arrays are 1-indexed, so every access into the state array
  d is at logical-position + 1. The state file is read into d with od and written
  back with printf once the emitted body has run.
**/
#include "ZshBackend.h"
...
//
// Fixed harness: read the state file into the 1-indexed array d, define
// register/memory/flag accessors, run the emitted body (%B%), then write d back.
// The "+1" on every index is the zsh 1-based-array adjustment.
//
static CHAR8 CONST *kTemplate =
    "d=($(od -An -v -tu1 \"$1\"))\n"
    "gR(){ local i=$1 b=$2 v=0 k; for ((k=0;k<b/8;k++)); do v=$(( v | (d[65536+i*8+k+1] << (8*k)) )); done; echo $v; }\n"
    "pR(){ local i=$1 v=$2 b=$3 k; for ((k=0;k<8;k++)); do if ((k<b/8)); then d[65536+i*8+k+1]=$(( (v>>(8*k))&0xff )); else d[65536+i*8+k+1]=0; fi; done; }\n"
    "rM(){ local a=$1 b=$2 v=0 k; for ((k=0;k<b/8;k++)); do v=$(( v | (d[a+k+1] << (8*k)) )); done; echo $v; }\n"
    "wM(){ local a=$1 v=$2 b=$3 k; for ((k=0;k<b/8;k++)); do d[a+k+1]=$(( (v>>(8*k))&0xff )); done; }\n"
    "gF(){ echo $(( d[65536+256+$1+1] & 1 )); }\n"
    "sF(){ d[65536+256+$1+1]=$(( $2 & 1 )); }\n"
    "sP(){ local p=$1 k; for ((k=0;k<8;k++)); do d[65536+264+k+1]=$(( (p>>(8*k))&0xff )); done; }\n"
...
public:
    ...
    CPU_EXEC_STATUS STDMETHODCALLTYPE Execute (VOID *pRAM, VOID *pGRF, VOID * /*pFRF*/) override {
        ...
        std::string Cmd = "zsh '" + m_Dir + "/insn.zsh' '" + State + "' 2>/dev/null";
...
public:
    ...
    ICpuCode *Build () {
        char DirTemplate[] = "/tmp/libcpu_zsh_XXXXXX";
        if (mkdtemp (DirTemplate) == nullptr) {
            return nullptr;
        }
        std::string Dir = DirTemplate;
        std::string Source = Dir + "/insn.zsh";
...
ICpuBackend *
CreateZshBackend (VOID)
{
    return new ZshBackend ();
}
