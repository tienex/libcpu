/** @file
  Korn shell textual-emit backend. See KshBackend.h.

  Each ICpuValue is a ksh integer variable (tN) masked to its width; arithmetic
  uses $(( ... )). ksh differs from bash in two ways used here: indexed arrays are
  built with "set -A" (not "d=(...)"), and function-local variables use "typeset"
  (ksh has no "local"). The state file is read into d with od and written back
  with printf once the emitted body has run.
**/
#include "KshBackend.h"
...
//
// Fixed harness: read the state file into the array d (set -A), define
// register/memory/flag accessors, run the emitted body (%B%), then write d back.
//
static CHAR8 CONST *kTemplate =
    "set -A d $(od -An -v -tu1 \"$1\")\n"
    "gR(){ typeset i=$1 b=$2 v=0 k; for((k=0;k<b/8;k++)); do v=$(( v | (d[65536+i*8+k] << (8*k)) )); done; echo $v; }\n"
    "pR(){ typeset i=$1 v=$2 b=$3 k; for((k=0;k<8;k++)); do if((k<b/8)); then d[65536+i*8+k]=$(( (v>>(8*k))&0xff )); else d[65536+i*8+k]=0; fi; done; }\n"
    "rM(){ typeset a=$1 b=$2 v=0 k; for((k=0;k<b/8;k++)); do v=$(( v | (d[a+k] << (8*k)) )); done; echo $v; }\n"
    "wM(){ typeset a=$1 v=$2 b=$3 k; for((k=0;k<b/8;k++)); do d[a+k]=$(( (v>>(8*k))&0xff )); done; }\n"
    "gF(){ echo $(( d[65536+256+$1] & 1 )); }\n"
    "sF(){ d[65536+256+$1]=$(( $2 & 1 )); }\n"
    "sP(){ typeset p=$1 k; for((k=0;k<8;k++)); do d[65536+264+k]=$(( (p>>(8*k))&0xff )); done; }\n"
    "sx(){ typeset v=$1 s=$2; if(( v & (1<<(s-1)) )); then echo $(( v-(1<<s) )); else echo $v; fi; }\n"
    "%B%"
    "e=$(printf '\\\\%03o' \"${d[@]}\")\n"
    "printf \"$e\" > \"$1\"\n";
...
class KshValue final : public LcComObject<ICpuValue> {
public:
    KshValue (UINT32 Id, UINT32 Bits) : m_Id (Id), m_Bits (Bits) {}
...
class KshBlock final : public LcComObject<ICpuBlock> {
public:
    HRESULT STDMETHODCALLTYPE QueryInterface (REFIID riid, VOID **ppvObject) override {
...
static UINT32 IdOf   (ICpuValue *pValue) { return static_cast<KshValue *> (pValue)->m_Id; }
static UINT32 BitsOf (ICpuValue *pValue) { return static_cast<KshValue *> (pValue)->m_Bits; }

class KshCode final : public LcComObject<ICpuCode> {
public:
    KshCode (std::string Dir) : m_Dir (std::move (Dir)) {}

    ~KshCode () override {
        if (!m_Dir.empty ()) {
            unlink ((m_Dir + "/insn.ksh").c_str ());
    ...
    CPU_EXEC_STATUS STDMETHODCALLTYPE Execute (VOID *pRAM, VOID *pGRF, VOID * /*pFRF*/) override {
        ...
        std::string Cmd = "ksh '" + m_Dir + "/insn.ksh' '" + State + "' 2>/dev/null";
...
class KshEmitter final : public LcComObject<ICpuEmitter> {
public:
    HRESULT STDMETHODCALLTYPE QueryInterface (REFIID riid, VOID **ppvObject) override {
    ...
    ICpuCode *Build () {
        char DirTemplate[] = "/tmp/libcpu_ksh_XXXXXX";
        if (mkdtemp (DirTemplate) == nullptr) {
            return nullptr;
        }
        std::string Dir = DirTemplate;
        std::string Source = Dir + "/insn.ksh";
...
ICpuBackend *
CreateKshBackend (VOID)
{
    return new KshBackend ();
}
