...
#define NIX_WIN32_ENOSYS_STUB(name, params) \
	static __inline int name params     \
	{                                   \
		errno = ENOSYS;             \
		return -1;                  \
	}
...
mprotect(void *addr, size_t len, int prot)
{
	...
	case 3:
		newp = PAGE_READWRITE;
		break; /* write implies read on win32 */
	case 4:
		newp = PAGE_EXECUTE;
		break;
	case 5:
