...
/* OS/2's kLIBC <sys/stat.h> #defines st_atimespec/st_mtimespec/st_ctimespec as BSD-compat aliases
   for st_atim/st_mtim/st_ctim. Those macros collide with libnix's OWN nix_stat field names of the
   same spelling, rewriting `out->st_atimespec` to `out->st_atim` (which nix_stat lacks). Drop the
   macros here -- only on OS/2, where they are macros rather than real host-struct fields, so other
   BSD hosts (where st_atimespec is a genuine struct member) are unaffected. */
#ifdef __OS2__
#undef st_atimespec
#undef st_mtimespec
#undef st_ctimespec
...
#if !defined(__linux__) && !defined(_WIN32) && !defined(__HAIKU__) && !defined(__OS2__)
#if defined(sun)
...
#endif
#if !defined(__linux__) && !defined(sun) && !defined(_WIN32) && !defined(__HAIKU__) && !defined(__OS2__)
...
#endif
#ifdef CSIZE
	/* An if/else chain, not a switch: on some hosts (e.g. Haiku) the CSn character-size constants
	   are not all distinct, which makes duplicate `case` labels a compile error. Comparisons are
	   first-match-wins, identical to the switch on hosts where the values differ. */
	{
		int _cs = flags & CSIZE;
		(void)_cs;
#ifdef CS5
...
#endif
		{ /* unrecognised character size: leave oflags unchanged */
		}
	}
#endif
#ifdef CSTOPB
                       ...
                       struct nix_termios   *out)
{
	...
	/* kLIBC (like Solaris) has no c_ispeed/c_ospeed members -- speed is via cfget/cfsetspeed. */
