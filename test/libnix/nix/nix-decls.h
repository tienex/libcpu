...
/* nix-env.c */
nix_env_t    *nix_env_create(nix_mem_if_t *mem);
void          nix_env_set_errno(nix_env_t *env, intmax_t error);
intmax_t      nix_env_get_errno(nix_env_t const *env);
nix_mem_if_t *nix_env_get_memory(nix_env_t const *env);
void          nix_env_set_memory(nix_env_t *env, nix_mem_if_t *memif);
/* nix-common.c */
...
nix_gaddr_t nix_mmap(nix_gaddr_t gaddr, size_t len, int prot, int flags, int fd, off_t offset, nix_env_t *env);
