...
{
	LCAssert0(count != 0);
	LCAssert0(count <= (sizeof(nix_sigset_t) << 3));

	g_sigcount = count;

	g_sigactions = nix_alloc_ntype(struct nix_sigaction, g_sigcount, 0);
	LCAssert0(g_sigactions != NULL);
...
{
	LCLog(g_LCLogImpl, LCLogDebug, 0, "pid=%d, signo=%d", pid, signo);
...
{
	LCLog(g_LCLogImpl, LCLogDebug, 0, "pgrp=%d, signo=%d", pgrp, signo);
...
{
	LCLog(g_LCLogImpl, LCLogDebug, 0, "signo=%d, sa=%p, osa=%p", signo, sa, osa);
...
{
	LCLog(g_LCLogImpl, LCLogDebug, 0, "signo=%d, ss=%p, oss=%p", signo, ss, oss);
...
nix_sigprocmask(int how, nix_sigset_t const *set, nix_sigset_t *oset,
                nix_env_t *env)
{
	LCLog(g_LCLogImpl, LCLogDebug, 0, "how=%d, set=%p, oset=%p", how, set, oset);
	...
	__nix_try
	{
		...
		if (set != NULL) {
			...
			case NIX_SIG_BLOCK:
				g_sigprocmask |= *set;
				break;
			case NIX_SIG_UNBLOCK:
				g_sigprocmask &= ~(*set);
				break;
			case NIX_SIG_SETMASK:
				g_sigprocmask = *set;
				break;
			default:
				nix_env_set_errno(env, EINVAL);
...
{
	LCLog(g_LCLogImpl, LCLogDebug, 0, "set=%p", set);
...
nix_sigtimedwait(nix_sigset_t const        *set,
                 ...
                 nix_env_t                 *env)
{
	LCLog(g_LCLogImpl, LCLogDebug, 0, "set=%p, info=%p, timeout=%p", set, info, timeout);
...
nix_setitimer(int                         timer,
              ...
              nix_env_t                  *env)
{
	LCLog(g_LCLogImpl, LCLogDebug, 0, "timer=%d, value=%p, ovalue=%p", timer, value, ovalue);
...
{
	LCLog(g_LCLogImpl, LCLogDebug, 0, "timer=%d, value=%p", timer, value);
...
nix_alarm(nix_time_t secs, nix_env_t *env)
{
	...
	LCLog(g_LCLogImpl, LCLogDebug, 0, "secs=%lld", secs);
...
nix_signal(int signo, uintmax_t handler, nix_env_t *env)
{
	...
	LCLog(g_LCLogImpl, LCLogDebug, 0, "signo=%u, handler=%llx", signo, handler);
