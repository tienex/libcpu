...
#define GELONG(x) ((sizeof(obsd41_long_t) == sizeof(uint64_t)) ? (obsd41_long_t)GE64(x) : (obsd41_long_t)GE32(x))

void
nix_timezone_to_obsd41_timezone(nix_endian_t               endian,
...
void
obsd41_timezone_to_nix_timezone(nix_endian_t                  endian,
...
void
nix_timespec_to_obsd41_timespec(nix_endian_t               endian,
...
void
obsd41_timespec_to_nix_timespec(nix_endian_t                  endian,
...
void
nix_timeval_to_obsd41_timeval(nix_endian_t              endian,
...
void
obsd41_timeval_to_nix_timeval(nix_endian_t                 endian,
...
nix_mode_to_obsd41_mode(nix_mode_t mode)
{
	...
	case NIX_S_IFIFO:
		result |= OBSD41_S_IFIFO;
		break;
	case NIX_S_IFCHR:
		result |= OBSD41_S_IFCHR;
		break;
	case NIX_S_IFDIR:
		result |= OBSD41_S_IFDIR;
		break;
	case NIX_S_IFBLK:
		result |= OBSD41_S_IFBLK;
		break;
	case NIX_S_IFREG:
		result |= OBSD41_S_IFREG;
		break;
	case NIX_S_IFLNK:
		result |= OBSD41_S_IFLNK;
		break;
	case NIX_S_IFSOCK:
		result |= OBSD41_S_IFSOCK;
		break;
	}

	if (mode & NIX_S_ISUID)
		result |= OBSD41_S_ISUID;
	if (mode & NIX_S_ISGID)
		result |= OBSD41_S_ISGID;
	if (mode & NIX_S_ISVTX)
		result |= OBSD41_S_ISVTX;

	return (result | (mode & 0777));
}

void
nix_stat_to_obsd41_stat(nix_endian_t           endian,
...
void
obsd41_sigaction32_to_nix_sigaction(nix_endian_t                     endian,
...
void
nix_sigaction_to_obsd41_sigaction32(nix_endian_t                endian,
...
void
nix_statfs_to_obsd41_statfs(nix_endian_t             endian,
...
static void
nix_sockaddr_in_to_obsd41_sockaddr_in(nix_endian_t                  endian,
...
static void
obsd41_sockaddr_in_to_nix_sockaddr_in(nix_endian_t                     endian,
...
static void
nix_sockaddr_un_to_obsd41_sockaddr_un(nix_endian_t                  endian,
...
static void
obsd41_sockaddr_un_to_nix_sockaddr_un(nix_endian_t                     endian,
...
int
nix_sockaddr_to_obsd41_sockaddr(nix_endian_t               endian,
...
int
obsd41_sockaddr_to_nix_sockaddr(nix_endian_t                  endian,
                                ...
                                nix_socklen_t                *outlen)
{
	switch (in->sa_family) {
	case NIX_AF_UNIX:
		LCAssert(g_bsd_log, inlen >= (obsd41_socklen_t)sizeof(struct obsd41_sockaddr_un));
	...
	case NIX_AF_INET:
		LCAssert(g_bsd_log, inlen >= (obsd41_socklen_t)sizeof(struct obsd41_sockaddr_in));
...
void
nix_rusage_to_obsd41_rusage(nix_endian_t             endian,
...
void
nix_rlimit_to_obsd41_rlimit(nix_endian_t             endian,
...
nix_poll_event_to_obsd41_event(int events)
{
	...
	if (events & NIX_POLLIN)
		oevents |= OBSD41_POLLIN;
	if (events & NIX_POLLPRI)
		oevents |= OBSD41_POLLPRI;
	if (events & NIX_POLLOUT)
		oevents |= OBSD41_POLLOUT;
	if (events & NIX_POLLRDNORM)
		oevents |= OBSD41_POLLRDNORM;
	if (events & NIX_POLLRDBAND)
		oevents |= OBSD41_POLLRDBAND;
	if (events & NIX_POLLWRBAND)
		oevents |= OBSD41_POLLWRBAND;
	if (events & NIX_POLLERR)
		oevents |= OBSD41_POLLERR;
	if (events & NIX_POLLHUP)
		oevents |= OBSD41_POLLHUP;
	if (events & NIX_POLLNVAL)
		oevents |= OBSD41_POLLNVAL;
...
obsd41_poll_event_to_nix_event(int16_t events)
{
	...
	if (events & OBSD41_POLLIN)
		xevents |= NIX_POLLIN;
	if (events & OBSD41_POLLPRI)
		xevents |= NIX_POLLPRI;
	if (events & OBSD41_POLLOUT)
		xevents |= NIX_POLLOUT;
	if (events & OBSD41_POLLRDNORM)
		xevents |= NIX_POLLRDNORM;
	if (events & OBSD41_POLLRDBAND)
		xevents |= NIX_POLLRDBAND;
	if (events & OBSD41_POLLWRBAND)
		xevents |= NIX_POLLWRBAND;
	if (events & OBSD41_POLLERR)
		xevents |= NIX_POLLERR;
	if (events & OBSD41_POLLHUP)
		xevents |= NIX_POLLHUP;
	if (events & OBSD41_POLLNVAL)
...
void
obsd41_pollfd_to_nix_pollfd(nix_endian_t                endian,
...
void
nix_pollfd_to_obsd41_pollfd(nix_endian_t             endian,
...
obsd41_termios_iflag_to_nix_termios_iflag(obsd41_tcflag_t flags)
{
	...
	if (flags & OBSD41_IGNBRK)
		oflags |= NIX_IGNBRK;
	if (flags & OBSD41_BRKINT)
		oflags |= NIX_BRKINT;
	if (flags & OBSD41_PARMRK)
		oflags |= NIX_PARMRK;
	if (flags & OBSD41_INPCK)
		oflags |= NIX_INPCK;
	if (flags & OBSD41_ISTRIP)
		oflags |= NIX_ISTRIP;
	if (flags & OBSD41_INLCR)
		oflags |= NIX_INLCR;
	if (flags & OBSD41_IGNCR)
		oflags |= NIX_IGNCR;
	if (flags & OBSD41_ICRNL)
		oflags |= NIX_ICRNL;
	if (flags & OBSD41_IXON)
		oflags |= NIX_IXON;
	if (flags & OBSD41_IXOFF)
		oflags |= NIX_IXOFF;
	if (flags & OBSD41_IXANY)
		oflags |= NIX_IXANY;
	if (flags & OBSD41_IUCLC)
		oflags |= NIX_IUCLC;
	if (flags & OBSD41_IMAXBEL)
		oflags |= NIX_IMAXBEL;
...
nix_termios_iflag_to_obsd41_termios_iflag(nix_tcflag_t flags)
{
	...
	if (flags & NIX_IGNBRK)
		oflags |= OBSD41_IGNBRK;
	if (flags & NIX_BRKINT)
		oflags |= OBSD41_BRKINT;
	if (flags & NIX_PARMRK)
		oflags |= OBSD41_PARMRK;
	if (flags & NIX_INPCK)
		oflags |= OBSD41_INPCK;
	if (flags & NIX_ISTRIP)
		oflags |= OBSD41_ISTRIP;
	if (flags & NIX_INLCR)
		oflags |= OBSD41_INLCR;
	if (flags & NIX_IGNCR)
		oflags |= OBSD41_IGNCR;
...
obsd41_termios_oflag_to_nix_termios_oflag(obsd41_tcflag_t flags)
{
	...
	if (flags & OBSD41_OPOST)
		oflags |= NIX_OPOST;
	if (flags & OBSD41_ONLCR)
		oflags |= NIX_ONLCR;
	if (flags & OBSD41_OXTABS)
		oflags |= NIX_OXTABS;
	if (flags & OBSD41_ONOEOT)
		oflags |= NIX_ONOEOT;
	if (flags & OBSD41_OCRNL)
		oflags |= NIX_OCRNL;
	if (flags & OBSD41_OLCUC)
		oflags |= NIX_OLCUC;
	if (flags & OBSD41_ONOCR)
		oflags |= NIX_ONOCR;
	if (flags & OBSD41_ONLRET)
		oflags |= NIX_ONLRET;
...
nix_termios_oflag_to_obsd41_termios_oflag(nix_tcflag_t flags)
{
	...
	if (flags & NIX_ONLCR)
		oflags |= OBSD41_ONLCR;
	if (flags & NIX_OXTABS)
		oflags |= OBSD41_OXTABS;
	if (flags & NIX_ONOEOT)
		oflags |= OBSD41_ONOEOT;
	if (flags & NIX_OCRNL)
		oflags |= OBSD41_OCRNL;
	if (flags & NIX_OLCUC)
		oflags |= OBSD41_OLCUC;
...
obsd41_termios_cflag_to_nix_termios_cflag(obsd41_tcflag_t flags)
{
	...
	if (flags & OBSD41_CIGNORE)
		oflags |= NIX_CIGNORE;
	if (flags & OBSD41_CSTOPB)
		oflags |= NIX_CSTOPB;
	if (flags & OBSD41_CREAD)
		oflags |= NIX_CREAD;
	if (flags & OBSD41_PARENB)
		oflags |= NIX_PARENB;
	if (flags & OBSD41_PARODD)
		oflags |= NIX_PARODD;
	if (flags & OBSD41_HUPCL)
		oflags |= NIX_HUPCL;
	if (flags & OBSD41_CLOCAL)
		oflags |= NIX_CLOCAL;
	if (flags & OBSD41_CRTSCTS)
		oflags |= NIX_CRTSCTS;
	if (flags & OBSD41_MDMBUF)
		oflags |= NIX_MDMBUF;
...
nix_termios_cflag_to_obsd41_termios_cflag(nix_tcflag_t flags)
{
	...
	if (flags & NIX_CIGNORE)
		oflags |= OBSD41_CIGNORE;
	if (flags & NIX_CSTOPB)
		oflags |= OBSD41_CSTOPB;
	if (flags & NIX_CREAD)
		oflags |= OBSD41_CREAD;
	if (flags & NIX_PARENB)
		oflags |= OBSD41_PARENB;
	if (flags & NIX_PARODD)
		oflags |= OBSD41_PARODD;
	if (flags & NIX_HUPCL)
		oflags |= OBSD41_HUPCL;
	if (flags & NIX_CLOCAL)
		oflags |= OBSD41_CLOCAL;
	if (flags & NIX_CRTSCTS)
		oflags |= OBSD41_CRTSCTS;
	if (flags & NIX_MDMBUF)
		oflags |= OBSD41_MDMBUF;
...
obsd41_termios_lflag_to_nix_termios_lflag(obsd41_tcflag_t flags)
{
	...
	if (flags & OBSD41_ECHOKE)
		oflags |= NIX_ECHOKE;
	if (flags & OBSD41_ECHOE)
		oflags |= NIX_ECHOE;
	if (flags & OBSD41_ECHOK)
		oflags |= NIX_ECHOK;
	if (flags & OBSD41_ECHO)
		oflags |= NIX_ECHO;
	if (flags & OBSD41_ECHONL)
		oflags |= NIX_ECHONL;
	if (flags & OBSD41_ECHOPRT)
		oflags |= NIX_ECHOPRT;
	if (flags & OBSD41_ECHOCTL)
		oflags |= NIX_ECHOCTL;
	if (flags & OBSD41_ISIG)
		oflags |= NIX_ISIG;
	if (flags & OBSD41_ICANON)
		oflags |= NIX_ICANON;
	if (flags & OBSD41_ALTWERASE)
		oflags |= NIX_ALTWERASE;
	if (flags & OBSD41_IEXTEN)
		oflags |= NIX_IEXTEN;
	if (flags & OBSD41_EXTPROC)
		oflags |= NIX_EXTPROC;
	if (flags & OBSD41_TOSTOP)
		oflags |= NIX_TOSTOP;
	if (flags & OBSD41_FLUSHO)
		oflags |= NIX_FLUSHO;
	if (flags & OBSD41_XCASE)
		oflags |= NIX_XCASE;
	if (flags & OBSD41_NOKERNINFO)
		oflags |= NIX_NOKERNINFO;
	if (flags & OBSD41_PENDIN)
		oflags |= NIX_PENDIN;
	if (flags & OBSD41_NOFLSH)
		oflags |= NIX_NOFLSH;
...
nix_termios_lflag_to_obsd41_termios_lflag(nix_tcflag_t flags)
{
	...
	if (flags & NIX_ECHO)
		oflags |= OBSD41_ECHO;
	if (flags & NIX_ECHONL)
		oflags |= OBSD41_ECHONL;
	if (flags & NIX_ECHOPRT)
		oflags |= OBSD41_ECHOPRT;
	if (flags & NIX_ECHOCTL)
		oflags |= OBSD41_ECHOCTL;
	if (flags & NIX_ISIG)
		oflags |= OBSD41_ISIG;
	if (flags & NIX_ICANON)
		oflags |= OBSD41_ICANON;
	if (flags & NIX_ALTWERASE)
		oflags |= OBSD41_ALTWERASE;
	if (flags & NIX_IEXTEN)
		oflags |= OBSD41_IEXTEN;
	if (flags & NIX_EXTPROC)
		oflags |= OBSD41_EXTPROC;
	if (flags & NIX_TOSTOP)
		oflags |= OBSD41_TOSTOP;
	if (flags & NIX_FLUSHO)
		oflags |= OBSD41_FLUSHO;
	if (flags & NIX_XCASE)
		oflags |= OBSD41_XCASE;
	if (flags & NIX_NOKERNINFO)
		oflags |= OBSD41_NOKERNINFO;
	if (flags & NIX_PENDIN)
		oflags |= OBSD41_PENDIN;
	if (flags & NIX_NOFLSH)
		oflags |= OBSD41_NOFLSH;
...
void
obsd41_termios_to_nix_termios(nix_endian_t                 endian,
...
void
nix_termios_to_obsd41_termios(nix_endian_t              endian,
...
void
obsd41_fd_set_to_nix_fd_set(nix_endian_t         endian,
...
void
nix_fd_set_to_obsd41_fd_set(nix_endian_t      endian,
                            nix_fd_set const *in,
                            obsd41_fd_set    *out)
{
	size_t n;
	...
	for (n = 0; n < sizeof(*in) / sizeof(nix_fd_mask_t) &&
	            n < sizeof(*out) / sizeof(obsd41_fd_mask_t);
	     n++) {
		out->fds_bits[n] = GE32(in->fds_bits[n]);
	}
}
