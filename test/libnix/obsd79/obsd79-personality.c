/*
 * obsd79-personality.c -- the OpenBSD/m88k personality behind the generic
...
/* Built in obsd79-init.c. */
extern nix_us_syscall_if_t *obsd79_us_syscall_create(nix_mem_if_t *memif);

/* m88k PSR carry (C) bit -- the BSD kernel's system-call error flag. */
#define OBSD79_PSR_CARRY (UINT32_C(1) << 28)

/* m88k register numbers this ABI assigns meaning to. */
enum {
	OBSD79_R_LINK = 1, /* r1  return/link */
	OBSD79_R_RET = 2,  /* r2  first arg / first return word */
	OBSD79_R_RET2 = 3, /* r3  second return word (dword split) */
	OBSD79_R_SP = 31,  /* r31 stack pointer */
	OBSD79_R_SCNO = 13 /* r13 system-call number */
};
...
enum { OBSD79_SYS_EXIT = 1,
       OBSD79_SYS_OBREAK = 17 };

typedef struct _obsd79_personality {
	...
	uint64_t                            brk_cur;
	int                                 inited;
} obsd79_personality_t;

/* Big-endian 32-bit store into guest memory at gaddr. */
static void
obsd79_wbe32(nix_mem_if_t *mem, uint64_t gaddr, uint32_t v)
{
	nix_memflg_t mf = 0;
...
static void
obsd79_ensure_init(obsd79_personality_t *self, nix_cpu_if_t *cpu)
{
	if (self->inited)
		return;
	...
	self->us = obsd79_us_syscall_create(cpu->mem);
 ...
 */
static void
obsd79_setup(nix_personality_t *base, nix_cpu_if_t *cpu,
             ...
             uint64_t brk_base)
{
	...
	a = sp;
	obsd79_wbe32(self->mem, a, (uint32_t)argc);
	a += 4;
	for (i = 0; i < argc; i++, a += 4)
		obsd79_wbe32(self->mem, a, (uint32_t)arg_addr[i]);
	obsd79_wbe32(self->mem, a, 0);
	a += 4;
	for (i = 0; i < nenv; i++, a += 4)
		obsd79_wbe32(self->mem, a, (uint32_t)env_addr[i]);
	obsd79_wbe32(self->mem, a, 0);

	nix_free(arg_addr);
	nix_free(env_addr);

	nix_cpu_reg_set(cpu, OBSD79_R_SP, sp);  /* r31 -> argc */
	nix_cpu_reg_set(cpu, OBSD79_R_LINK, 0); /* crt0 calls exit(); link unused */
}
...
obsd79_syscall(nix_personality_t *base, nix_cpu_if_t *cpu)
{
	...
	scno = (int)self->ctx.gpr[OBSD79_R_SCNO];

	if (getenv("LCX_STRACE") != NULL)
		fprintf(stderr, "lcx strace: r13=%d (%#x, %#x, %#x)\n", scno,
		        self->ctx.gpr[OBSD79_R_RET], self->ctx.gpr[3],
		        self->ctx.gpr[4]);

	if (scno == OBSD79_SYS_EXIT)
		return (0); /* stop the guest */

	if (scno == OBSD79_SYS_OBREAK) {
		/* A flat-RAM model has no page tables; brk just bumps a pointer
		 * and validates it against RAM. */
		uint64_t addr = self->ctx.gpr[OBSD79_R_RET];

		if (addr == 0) {
			nix_cpu_reg_set(cpu, OBSD79_R_RET, self->brk_cur);
			nix_cpu_set_carry(cpu, 0);
		} else if (addr < self->brk_base || addr >= self->ram_size - 0x10000) {
			nix_cpu_reg_set(cpu, OBSD79_R_RET, (uint64_t)(uint32_t)ENOMEM);
			nix_cpu_set_carry(cpu, 1);
		} else {
			self->brk_cur = addr;
			nix_cpu_reg_set(cpu, OBSD79_R_RET, addr);
			nix_cpu_set_carry(cpu, 0);
		}
		return (1);
	}
	...
	nix_cpu_reg_set(cpu, OBSD79_R_RET, self->ctx.gpr[OBSD79_R_RET]);
	nix_cpu_reg_set(cpu, OBSD79_R_RET2, self->ctx.gpr[OBSD79_R_RET2]);
	nix_cpu_set_carry(cpu, (self->ctx.psr & OBSD79_PSR_CARRY) ? 1 : 0);
	return (1);
}

static void
obsd79_destroy(nix_personality_t *base)
{
	nix_free(base);
}

static struct _nix_personality_vtbl const obsd79_personality_vtbl = {
    obsd79_setup,
    obsd79_syscall,
    obsd79_destroy};
...
{
	obsd79_personality_t *self =
	    (obsd79_personality_t *)nix_alloc_type(obsd79_personality_t, 0);

	if (self != NULL)
		self->vtbl = &obsd79_personality_vtbl;
